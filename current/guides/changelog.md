...
- fix(Devtools): edge hit flag ignoring re-entering x-layer0-t values (#745) @adrien-k
...
- fix x-layer0-destination destinationName escape in edge_recv_evaluate_de… (#673) @KaarelKelk
...
- Log x-layer0-hit-request-id (#583) @ierceg
...
- Add x-layer0-user-t to server-timing response header (#484) @ierceg
...
- Optimize x-layer0-original-qs to be injected only when qs are changed (#468) @ierceg
...
- Inject example x-layer0-geo- headers when running locally for easier testing. (#460) @phdunham
...
- Emulate edge's x-layer0-client-ip header when running locally (#454) @phdunham
